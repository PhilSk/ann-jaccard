...
class NmslibReuseIndex(BaseANN):
    ...
    def __init__(self, metric, method_name, index_param, save_index,
                 query_param):
        self._nmslib_metric = {
            'angular': 'cosinesimil',
            'euclidean': 'l2'
        ...
        self._index_name = os.path.join(INDEX_DIR, "nmslib_%s_%s_%s" %
                                        (self._method_name, metric,
                                         '_'.join(self._index_param)))
...
class NmslibNewIndex(BaseANN):
    def __init__(self, metric, method_name, method_param):
        ...
        self.name = 'Nmslib(method_name=%s, method_param=%s)' % (
