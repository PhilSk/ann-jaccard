...
def knn(dataset_distances, run_distances, count, epsilon=1e-1000):
    ...
    for true_distances, found_distances in zip(dataset_distances,
        ...
        # print('///////////////////////////')
        within = [
            d for d in found_distances[:count]
            if d >= (true_distances)[-100] - epsilon
        ]
        actual += len(within)
...
def epsilon(dataset_distances, run_distances, count, epsilon=0.01):
    ...
    for true_distances, found_distances in zip(dataset_distances,
        ...
        within = [
            d for d in found_distances[:count]
            if d <= true_distances[count - 1] * (1 + epsilon)
        ]
        actual += len(within)
    return float(actual) / float(total)
