...
def compute_metrics(true_nn_distances, res, metric_1, metric_2):
    ...
    for i, (definition, run) in enumerate(res):
        ...
        metric_1_value = metrics[metric_1]['function'](
            true_nn_distances, run_distances, run.attrs)
        metric_2_value = metrics[metric_2]['function'](
            true_nn_distances, run_distances, run.attrs)

        print('%3d: %80s %12.3f %12.3f' % (i, algo_name, metric_1_value,
                                           metric_2_value))
...
def create_linestyles(unique_algorithms):
    ...
    return dict((algo, (colors[algo], faded[algo], linestyles[algo],
                        markerstyles[algo])) for algo in unique_algorithms)
